...
def create_triplet(dataset, n_classes=3, n_support=2, n_query=2):
    ...
    for class_ in selected_classes:
        # Selecting anchor, positive, and negative samples
        anchor_samples = random.sample(dataset[class_], n_support)
        positive_samples = random.sample(dataset[class_], n_support)
        negative_class = random.choice(list(set(selected_classes) - set([class_])))
        negative_samples = random.sample(dataset[negative_class], n_support)

        for anchor, positive, negative in zip(anchor_samples, positive_samples, negative_samples):
            triplets.append((anchor, positive, negative))

    return triplets, class_label_mapping
...
def train_model_triplet(model, optimizer, triplet_loss, dataset, n_epochs, n_episodes, n_classes, n_support, n_query):
    for epoch in range(n_epochs):
        epoch_loss, epoch_acc = 0.0, 0.0
        for episode in range(n_episodes):
            triplets, class_label_mapping = create_triplet(dataset, n_classes, n_support, n_query)

            anchor_landmarks = torch.tensor([landmarks for anchor, _, _ in triplets], dtype=torch.float32).view(-1,
                                                                                                                len(
                                                                                                                    triplets[
                                                                                                                        0][
                                                                                                                        0]),
                                                                                                                2)
            positive_landmarks = torch.tensor([landmarks for _, positive, _ in triplets], dtype=torch.float32).view(-1,
                                                                                                                    len(
                                                                                                                        triplets[
                                                                                                                            0][
                                                                                                                            1]),
                                                                                                                    2)
            negative_landmarks = torch.tensor([landmarks for _, _, negative in triplets], dtype=torch.float32).view(-1,
                                                                                                                    len(
                                                                                                                        triplets[
                                                                                                                            0][
                                                                                                                            2]),
            ...
            anchor_embeddings = model(anchor_landmarks).mean(1)
            positive_embeddings = model(positive_landmarks).mean(1)
            negative_embeddings = model(negative_landmarks).mean(1)
...
def predict(model, dataset, new_data, n_classes=3, n_support=2,n_query=2):
    support_samples, _, class_label_mapping = create_triplet(dataset, n_classes, n_support, n_query)  # get the mapping
...
if __name__ == "__main__":
    ...
    train_model_triplet(model, optimizer, criterion, dataset, n_epochs=60, n_episodes=200, n_classes=3, n_support=2, n_query=2)


    #new_data = load_data('/Users/hwangjiyeong/PycharmProjects/facial reconition/face_recognition_1/test_data/landmark_test_data')
